...
### Script usage: sh slurm_buildtitleregex_body.sh. In order to use different
...
do
 ...
 echo "#SBATCH -J stbo${i}" >> $TEMPFILE
 echo "#SBATCH -t 14-00:00" >> $TEMPFILE
 echo "#SBATCH --wckey=marxnfs1" >> $TEMPFILE
 echo "" >> $TEMPFILE
 echo "perl buildtitleregex_byyear_body.pl $dBase $i" >> $TEMPFILE
sbatch $TEMPFILE
sleep 0.1
done
