...
if ($ARGV[0]=~/^wos$/i) {
    ...
    if (!(-e $inputfile)) {
	die("Usage: buildtitleregex_1799_front.pl oa|mag|wos|filename_or_fullpath_of_file\n");
    }
}
...
while (<INFILE>) {
	...
	if (($firstauthor_lastname eq "")||($firstauthor_lastname eq "[anonymous]")||($year<1799)||($year>2022)) { next; }
...
#	print "$longestword - $secondlongestword --- $mytitle\n";
	...
	if (($longestword)&&(length($longestword)>=2)) {
		...
		#$regex=~s/[\$]//g; # GET RID OF DOLLAR SIGNS; SHOULD BE IN CLEANPARAS
