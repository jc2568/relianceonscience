...
### Script usage: sh slurm_buildtitleregex_front.sh. In order to use different
...
do
 ...
 echo "#SBATCH -J stfo$i" >> $TEMPFILE
 echo "#SBATCH -t 14-00:00" >> $TEMPFILE
 echo "#SBATCH --wckey=marxnfs1" >> $TEMPFILE
 echo "" >> $TEMPFILE
 echo "perl buildtitleregex_byyear_front.pl $dBase $i" >> $TEMPFILE
sbatch $TEMPFILE
sleep 0.1
done
