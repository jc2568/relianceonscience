...
cat $fullpath/cleanparas-10* | sort -u | tr [:upper:] [:lower:] | perl $scripts/screenfrontjunk.pl >> $fullpath/$CURRENTYR.tsv
chmod 777 $fullpath/$CURRENTYR.tsv

echo "identifying citations with no years"
perl $scripts/frontnoyear.pl $fullpath/$CURRENTYR.tsv | grep '^__'  >> $fullpath/frontbyrefyear/front_1799.tsv
...
for ((i=1800; i<=$CURRENTYR; i++))
 ...
 echo "" >> $TEMPFILE
 echo "cat $fullpath/$CURRENTYR.tsv |  grep "[^a-z0-9\-]$i[^0-9\-]" > $fullpath/frontbyrefyear/front_$i.tsv" >> $TEMPFILE
