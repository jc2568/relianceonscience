...
#open(PREFIXES,"../prefixes.txt")||die("can't find prefixes file.\n");
$prefixes="$INPUTDIR_PATENT_DEPENDENCIES" . "prefixes.txt";
open(PREFIXES,$prefixes)||die("can't find prefixes file.\n");
...
#open(POSTFIXES,"../postfixes.txt")||die("can't find postfixes file.\n");
$postfixes="$INPUTDIR_PATENT_DEPENDENCIES" . "postfixes.txt";
open(POSTFIXES,$postfixes)||die("can't find postfixes file.\n");
...
#open(JOURNALABBREVS,"../../journalabbrev/journalabbrevs-extended.tsv")||die("can't find journals file\n");
$journals="$INPUTDIR_PATENT_DEPENDENCIES" . "journalabbrevs-extended.tsv";
open(JOURNALABBREVS,$journals)||die("can't find journalabbrevs-extended.tsv file.\n");
...
#open(POSTYEARWORDS,"../postyearwords.txt")||die("Can't open postyearwords.txt\n");
$postyearwords="$INPUTDIR_PATENT_DEPENDENCIES" . "postyearwords.txt";
open(POSTYEARWORDS,$postyearwords)||die("Can't open postyearwords.txt\n");
...
#open(PREYEARWORDS,"../preyearwords.txt")||die("Can't open preyearwords.txt\n");
$preyearwords="$INPUTDIR_PATENT_DEPENDENCIES" . "preyearwords.txt";
open(PREYEARWORDS,$preyearwords)||die("Can't open preyearwords.txt\n");
...
#    if (/(^_?_?[A-Z]{0,4}\d{1,10})\t(\d+)\t(\d+)\t(.*)/) { ### the original line in USPTO only
#    if (/(^__[^\t]+)\t(\d+)\t(\d+)\t(.*)/) { ### the script runs "properly" when -A2 is not present in patent number EPO/USPTO
    if (/(^__[^\t]+)\t(\d+)\t(\d+)\t(.*)/) {
#    if (/(^__.*)/) { ### tried to use but failed; to generic
	$patent = $1;
	$year = $2;
	$originaldocoffset = $3;
	$line = $4;
