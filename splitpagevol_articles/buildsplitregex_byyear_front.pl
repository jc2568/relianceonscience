...
if ($ARGV[0]=~/^wos$/i) {
    ...
    if (!$inputyear) {
	die("Usage: buildtitleregex_byyear_front.pl oa YEAR|mag YEAR|wos YEAR|filename_or_fullpath_of_file\n");
    }
}
elsif ($ARGV[0]=~/^mag$/i) {
    ...
    if (!$inputyear) {
	die("Usage: buildsplitregex_front_byyear.pl oa YEAR|mag YEAR|wos YEAR|filename_or_fullpath_of_file\n");
    }
}
elsif ($ARGV[0]=~/^oa$/i) {
    ...
    if (!(-e $inputfile)) {
	die("Usage: buildsplitregex_byyear_front.pl oa YEAR|mag YEAR |wos YEAR|filename_or_fullpath_of_file\n");
    }
}
