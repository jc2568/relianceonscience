...
if ($ARGV[0]=~/^wos$/i) {
    ...
    if (!$inputyear) {
	die("Usage: buildtitleregex_byyear_body.pl oa YEAR|mag YEAR|wos YEAR|filename_or_fullpath_of_file\n");
    }
}
elsif ($ARGV[0]=~/^mag$/i) { 
    ...
    if (!$inputyear) {
	die("Usage: buildsplitregex_byyear_body.pl oa YEAR|mag YEAR|wos YEAR|filename_or_fullpath_of_file\n");
    }
}
elsif ($ARGV[0]=~/^oa$/i) {
...
#		    (($digit==1)||($digit==3))&&($sizect>=($filesize/24))) {
		# Make this even more extreme (1/24) for the single digits 1 and 3 which have much larger files than other numbers.
			...
			close(OUTFILE); 
#			`chmod 777 $outputfile`; this line was the original line that cause a chmod error and prevented files
#                        from having the correct permissions. This line was moved 6 lines down in the script.
		    }
