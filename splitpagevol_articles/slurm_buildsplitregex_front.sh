...
### Script usage: sh slurm_buildsplitregex_byyear_front.sh. In order to use different
...
do
 ...
 echo "buildsplitregex_byyear_front $i"
 echo "#!/bin/bash" > $TEMPFILE
 echo "#SBATCH -p large,xlarge" >> $TEMPFILE
 echo "#SBATCH -J pvfo$i" >> $TEMPFILE
 echo "#SBATCH -t 14-00:00" >> $TEMPFILE
 echo "#SBATCH --wckey=marxnfs1" >> $TEMPFILE
 echo "" >> $TEMPFILE
 echo "perl buildsplitregex_byyear_front.pl $dBase $i" >> $TEMPFILE
sbatch $TEMPFILE
sleep 0.1
done
