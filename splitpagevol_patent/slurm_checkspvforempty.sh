...
### Command structure: sh slurm_checkspvforempty.sh
...
do
 ...
 echo "#SBATCH -J ckpv$i" >> $TEMPFILE
 echo "#SBATCH -t 96:00:00" >> $TEMPFILE
 echo "#SBATCH --wckey=marxnfs1" >> $TEMPFILE
 echo "" >> $TEMPFILE
 echo "module load perl5-libs" >>$TEMPFILE
 echo "python3 $directory/checkspvforempty.py $i $frontorintext > ${frontorintext}_$i.tsv" >> $TEMPFILE
 sbatch $TEMPFILE
 sleep 0.1
done
